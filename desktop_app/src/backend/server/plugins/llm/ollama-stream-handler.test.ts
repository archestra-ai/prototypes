...
describe('handleOllamaStream', () => {
  ...
  describe('Text-only streaming', () => {
    it('should handle simple text response', async () => {
      ...
      expect(events[1]).toMatchObject({ type: 'text-start' });
      expect(events[2]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'Hello' });
      expect(events[3]).toEqual({ type: 'text-delta', id: expect.any(String), delta: ' there' });
    ...
    it('should skip empty/whitespace-only content', async () => {
      ...
      expect(events[3]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '   ' });
      expect(events[4]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '\n\n' });
      expect(events[5]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'World' });
    ...
    it('should handle think blocks in text', async () => {
      ...
      expect(events[0]).toEqual({ type: 'start' });
      expect(events[1]).toMatchObject({ type: 'text-start' });
      expect(events[2]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '<think>' });
      expect(events[3]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'This is my thought process' });
      expect(events[4]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '</think>' });
  ...
  describe('Tool-only streaming', () => {
    it('should handle tool call without text', async () => {
      ...
      expect(events[5]).toEqual({ type: 'finish' });
    });
    ...
    it('should handle multiple tool calls', async () => {
      ...
      // First tool call - all events in sequence
      expect(events[1]).toMatchObject({ type: 'tool-input-start', toolName: 'echo' });
      expect(events[2]).toMatchObject({ type: 'tool-input-delta', inputTextDelta: '{"message":"First"}' });
      expect(events[3]).toMatchObject({ type: 'tool-input-available', input: { message: 'First' } });
      expect(events[4]).toMatchObject({ type: 'tool-output-available' });

      // Second tool call - all events in sequence
      expect(events[5]).toMatchObject({ type: 'tool-input-start', toolName: 'echo' });
      expect(events[6]).toMatchObject({ type: 'tool-input-delta', inputTextDelta: '{"message":"Second"}' });
      expect(events[7]).toMatchObject({ type: 'tool-input-available', input: { message: 'Second' } });
      expect(events[8]).toMatchObject({ type: 'tool-output-available' });
  ...
  describe('Mixed text and tool streaming', () => {
    it('should send text-end before tool-input-start', async () => {
      ...
      expect(events[0]).toEqual({ type: 'start' });
      expect(events[1]).toMatchObject({ type: 'text-start' });
      expect(events[2]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'Let me help you with that.' });
      expect(events[3]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '\n\n' });
      expect(events[4]).toMatchObject({ type: 'text-end' }); // Should come before tool events
      expect(events[5]).toMatchObject({ type: 'tool-input-start', toolName: 'echo' });
      expect(events[6]).toMatchObject({ type: 'tool-input-delta' });
      expect(events[7]).toMatchObject({ type: 'tool-input-available' });
      expect(events[8]).toMatchObject({ type: 'tool-output-available' });
      expect(events[9]).toEqual({ type: 'finish' });
    });
    ...
    it('should handle text with think block followed by tool call', async () => {
      ...
      expect(events[1]).toMatchObject({ type: 'text-start' });
      expect(events[2]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '<think>' });
      expect(events[3]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'I need to use a tool for this' });
      expect(events[4]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '</think>' });
      expect(events[5]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '\n\n' });
      expect(events[6]).toMatchObject({ type: 'text-end' }); // Must come before tool events
      expect(events[7]).toMatchObject({ type: 'tool-input-start' });
      expect(events[8]).toMatchObject({ type: 'tool-input-delta' });
      expect(events[9]).toMatchObject({ type: 'tool-input-available' });
      expect(events[10]).toMatchObject({ type: 'tool-output-available' });
      expect(events[11]).toEqual({ type: 'finish' });
    });
    ...
    it('should handle tool execution at chunk.done', async () => {
      ...
      expect(events[0]).toEqual({ type: 'start' });
      expect(events[1]).toMatchObject({ type: 'text-start' });
      expect(events[2]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'Processing your request' });
      expect(events[3]).toEqual({ type: 'text-delta', id: expect.any(String), delta: '...' });
      expect(events[4]).toMatchObject({ type: 'text-end' }); // Should close text before tools
      expect(events[5]).toMatchObject({ type: 'tool-input-start' });
      expect(events[6]).toMatchObject({ type: 'tool-input-delta' });
      expect(events[7]).toMatchObject({ type: 'tool-input-available' });
      expect(events[8]).toMatchObject({ type: 'tool-output-available' });
      expect(events[9]).toEqual({ type: 'finish' });
  ...
  describe('Error handling', () => {
    it('should handle tool execution errors', async () => {
      ...
      expect(events[0]).toEqual({ type: 'start' });
      expect(events[1]).toMatchObject({ type: 'tool-input-start' });
      expect(events[2]).toMatchObject({ type: 'tool-input-delta' });
      expect(events[3]).toMatchObject({ type: 'tool-input-available' });
      expect(events[4]).toMatchObject({
    ...
    it('should handle invalid tool arguments', async () => {
      ...
      expect(events[4]).toEqual({ type: 'finish' });
    });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle no MCP tools available', async () => {
      ...
      expect(events[0]).toEqual({ type: 'start' });
      expect(events[1]).toMatchObject({ type: 'text-start' });
      expect(events[2]).toEqual({ type: 'text-delta', id: expect.any(String), delta: 'Hello without tools' });
      expect(events[3]).toMatchObject({ type: 'text-end' });
      expect(events[4]).toEqual({ type: 'finish' });
    });
